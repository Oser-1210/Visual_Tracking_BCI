...
class NeuroScanEEGThread(threading.Thread,):
    def __init__(self, config=None, hostname='0.0.0.0', port=4000, threadName='NeuroScanEEG', device='NeuroScan', n_chan=2, record_srate=1000,srate=250):
        ...
        # cash 数据和event缓存
        self.event = None
        self.eventPoint = deque(maxlen=10000)
        ...
        self.messageQueue = deque(maxlen=100000)  # 缓冲的数据队列
    ...
    def readFixedData(self, startPoint, length):
        ...
        while self.eventExist is True:
            ...
            point, packageInx, eventType = events
    ...
    def _addEvent(self):
        ...
        if self.event is not None:
            ...
            if eventType <100:
                ...
                self.eventPoint.appendleft([point, packageINX, eventType])
                self.event = None
                self.eventExist = True
            else:
                self.eventINX += 1
                print('epoch number: %d' % self.eventINX)
                self.event = None
