...
class ExchangeMessageManagement:
    def __init__(self,role,exchange_message_operator,config):
        ...
        self.hostname = config.client_ip
    ...
    def initial(self, exchange_message_operator):
        ...
        if self.connect():
            ...
            self.receive_message_thread = Thread(name='recieve',target=receive_fcn, args=(self.message_queue, self.clientSocket, lambda: self.stop_flag,))

            self.operator_message_thread = Thread(name='operate',target=operate_fcn, args=(self.message_queue, self.event_manager, lambda: self.stop_flag,))
    ...
    def stop(self):
        ...
        self.receive_message_thread.join()
